"""Win overlay handicapper: last-run figure + sectionals → probabilities."""
...
def _draw_bias_lbs(venue: str, distance_m: float, barrier: int | None, field_size: int) -> float:
    if not barrier or field_size < 8:
        return 0.0
    tight = any(v in (venue or "").lower() for v in ("nowra", "armidale", "bairnsdale", "kalgoorlie"))
    if not tight:
        return 0.0
